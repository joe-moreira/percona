...
func updatesAndGrants() {
	...
	fmt.Println("\n✅  MySQL is open for connections.")
	fmt.Println("\n✅ Now execute: mysql -uroot -p   <with the pwd provided>")
	fmt.Println("\n✅ SET GLOBAL validate_password_policy = LOW;")
	fmt.Println("\n✅ UPDATE mysql.user SET authentication_string = PASSWORD('wejk23#1'), password_expired = 'N' WHERE User = 'root' AND Host = 'localhost';"
	fmt.Println("\n✅ FLUSH PRIVILEGES;")
	...
	/*
		...
		_, err3 := db.Exec("SET GLOBAL validate_password_policy = LOW;")
		if err3 != nil {
		...
		_, err4 := db.Exec("SET PASSWORD = PASSWORD('wejk23#1')")
		//update, err4 := db.Exec("UPDATE mysql.user SET authentication_string = PASSWORD('wejk23#1'), password_expired = 'N' WHERE User = 'root' AND Host = 'localhost';")
		if err4 != nil {
		...
		_, err5 := db.Exec("FLUSH PRIVILEGES;")
		if err5 != nil {
...
func functionsCreation() {
	...
	_, err2 := db.Exec("CREATE FUNCTION fnv1a_64 RETURNS INTEGER SONAME 'libfnv1a_udf.so';")
	if err2 != nil {
		fmt.Println("\n❌  Couldn't create the first function.")
		panic(err2.Error())
	}

	_, err3 := db.Exec("CREATE FUNCTION fnv_64 RETURNS INTEGER SONAME 'libfnv_udf.so'")
	if err3 != nil {
		fmt.Println("\n❌  Couldn't create the second function.")
		panic(err3.Error())
	}

	_, err4 := db.Exec("CREATE FUNCTION murmur_hash RETURNS INTEGER SONAME 'libmurmur_udf.so'")
	if err4 != nil {
